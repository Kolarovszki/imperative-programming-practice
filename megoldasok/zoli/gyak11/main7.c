...
StudentType* get_student_with_max_creditindex(StudentType** array, int d) {
    int i;

    double max_creditindex = array[0]->creditindex;

    StudentType* student_with_max_creditindex;
    ...
    for (i = 0; i < d; i++){
        ...
        if (current_student->creditindex > max_creditindex) {
            ...
            student_with_max_creditindex = current_student;
        }
    }

    return student_with_max_creditindex;
}
...
int main() {
    ...
    StudentType* student_with_max_creditindex = get_student_with_max_creditindex(student_tomb, 3);

    print_student(student_with_max_creditindex);
